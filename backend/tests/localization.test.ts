...
import { getTopologyForDT, getDownstreamPoles, getUpstreamPath, clearTopologyCache, InferredTopology } from "../src/topology.js";
...
describe("topology traversal helpers", () => {
  const sampleTopo: InferredTopology = {
    dtId: "D-TEST",
    known: true,
    confidence: 0.9,
    poles: [
...
describe("geometric topology inference", () => {
  it("produces a connected-ish graph on collinear points", () => {
    clearTopologyCache();
    const inferred: InferredTopology = {
      dtId: "D-GEO",
      known: false,
      confidence: 0.7,
      poles: [],
